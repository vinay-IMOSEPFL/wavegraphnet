...
class ForwardPhysicsGNN(nn.Module):
    """
    Predicts path-wise energy deviation (Delta E)[cite: 177, 314].
    """

    def __init__(self, edge_in=6, hidden=128):
        super().__init__()
        ...
        self.out_layer = nn.Sequential(
            nn.Linear(hidden, 64),
            nn.ReLU(),
            nn.Linear(64, 1),  # Predicts one Delta E value per path [cite: 179]
        )
    ...
    def forward(self, data, pred_coords):
        ...
        # Standard geometric features used in WaveGraphNet [cite: 271]
        ...
        # Path-to-damage perpendicular distance logic
        ...
        # Simple residual pass
        h_e = F.relu(self.res_linear(h_e) + h_e)

        return self.out_layer(h_e).squeeze(-1)
